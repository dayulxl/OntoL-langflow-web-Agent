/**
 * 本体建模 — Memgraph 图可视化编辑器
 * 功能: 工具栏 + 彩色节点画布 + 右侧属性栏 + 场景管理
 */
...
export default function OntolModelingPage() {
  const [nodes, setNodes, onNodesChange] = useNodesState<Node>([]);
  const [edges, setEdges, onEdgesChange] = useEdgesState<Edge>([]);
  const [selectedNode, setSelectedNode] = useState<Node | null>(null);
  const [selectedEdge, setSelectedEdge] = useState<Edge | null>(null);
  const [sidebarMode, setSidebarMode] = useState<"detail" | "createNode" | "createEdge" | "scene" | null>(null);
  ...
  return (
    <div className="flex h-[calc(100vh-48px)] bg-[#0a0f1e]">
      ...
      <div className="flex-1 relative">
        {/* 工具栏 */}
        <div className="absolute top-3 left-3 z-20 flex items-center gap-2 bg-[#0a0f1ee8] backdrop-blur border border-white/10 rounded-lg px-3 py-2">
...
function ToolBtn({ onClick, danger, children }: any) {
  return (
